...
from fellowme_msgs.srv import *

class move_base_sequence():
    def __init__(self):
        rospy.init_node('move_base_seq_server')
        ...
        topics_prefix = rospy.get_param("fellowme/move_base_seq_server/topics_prefix","fellowme/fellowme_navigation")
        _ = rospy.Subscriber(topics_prefix+"/move_base_sequence/wayposes",PoseArray,self.set_poses)          
        _ = rospy.Subscriber(topics_prefix+"/move_base_sequence/add_goal_pose",PoseStamped,self.add_pose)          
        _ = rospy.Service(topics_prefix+'/move_base_sequence/get_state', get_state, self.get_state) 
        _ = rospy.Service(topics_prefix+'/move_base_sequence/toggle_state', toggle_state, self.toggle_state) 
        _ = rospy.Service(topics_prefix+'/move_base_sequence/set_state', set_state, self.set_state) 
        _ = rospy.Service(topics_prefix+'/move_base_sequence/reset', reset, self.reset) 
        ...
        if not wait:
            ...
            return
        rospy.loginfo("Connected to move base server, moving the base "+ ("one way trip" if rospy.get_param("/move_base_seq_server/is_repeating",True) == False else "in a repetitive way"))
    ...
    def done_cb(self, status, result):
        ...
        if status == 2:
            ...
            behav = rospy.get_param("/move_base_seq_server/abortion_behaviour","stop")
            if behav == "stop": self.__state__ = False
            elif not (behav == "continue"): rospy.logwarn("Param /move_base_seq_server/abortion_behaviour  is neither 'stop' nor 'continue'! continue is assumed.")
    ...
    def _set_next_goal(self):
        ...
        if self._i <= len(self.poses.poses)-1: pass
            ...
            if rospy.get_param("/move_base_seq_server/is_repeating",True):
               self._i = 0
