...
#include <fellowme_msgs/PIDStamped.h>
...
namespace fellowme {
     ...
     * 
     * The BaseController communicates with the high level interface fellowme_base::hardware_interface::RobotHW
     ...
     * (\ref fellowme_msgs::WheelsCmdStamped on the "wheel_cmd_velocities" topic) with the 
     ...
     * \ref fellowme::MotorControllerIntf. Each time a new \ref fellowme_msgs::WheelsCmdStamped is received on the
     ...
     * two \ref fellowme::Encoder objects (\ref encoder_left_ \ref encoder_right_)
     * are used to \ref read() the fellowme::JointStates, stored in \ref joint_state_left_ and \ref joint_state_left_.
     * After reading the latest states (position and velocity), the values are published with \ref pub_measured_joint_states_ on 
     * "measured_joint_states" topic of type \ref sensor_msgs::JointState. The \ref fellowme_base::hardware_interface::RobotHW
     ...
     * - /fellowme/encoder_resolution
     * - /fellowme/mobile_base_controller/wheel_radius
     * - /fellowme/mobile_base_controller/linear/x/max_velocity
     * - /fellowme/debug/base_controller
     * 
     */
    ...
    public:
         ...
         * The only requirement is to implement the \ref fellowme::MotorControllerIntf,
         ...
         */
        BaseController(ros::NodeHandle &nh, L298NController* motor_controller_left, L298NController* motor_controller_right);
         ...
         *   - /fellowme/mobile_base_controller/linear/x/max_velocity
         *   - /fellowme/debug/base_controller
         * - Initialize fellowme Wheel Encoders
         ...
         * \ref LastUpdateTime::command_received period on the /fellowme/wheel_cmd_velocities topic.
         * 
         */
        void eStop();
         ...
         * (e.g. fellowme::PID::error_ = \ref wheel_cmd_velocity_left_ - \ref measured_angular_velocity_left_)
         * 
         * The calculated PID ouput values are capped at -/+ MAX_RPM to prevent the PID from having too much error
         * The computed and capped PID values are then set using the \ref fellowme::MotorControllerIntf::setSpeed method
        ...
        /**
         * @brief Callback method when a new \ref fellowme_msgs::WheelsCmdStamped is received on the wheel_cmd_velocities topic.
         * 
         ...
         * 
         * This callback method receives \ref fellowme_msgs::WheelsCmdStamped message object (\p cmd_msg) consisting of
         * \ref fellowme_msgs::AngularVelocities, where commanded wheel joints for the left and right wheel are stored.
         ...
         * This is used for the eStop functionality. In case no fellowme_msgs::WheelsCmdStamped messages are 
         ...
         */
        void commandCallback(const fellowme_msgs::WheelsCmdStamped& cmd_msg);
         ...
         * Every time the fellowme_bringup/launch/bringup.launch is launched, the high level
         ...
         */
        void pidLeftCallback(const fellowme_msgs::PIDStamped& pid_msg);
         ...
         * @param pid_msg message containing the new PID constants
         */
        void pidRightCallback(const fellowme_msgs::PIDStamped& pid_msg);
    ...
    private:
        ...
        EncoderHandler encoder_left_;
        EncoderHandler encoder_right_;
        long ticks_left_ = 0, ticks_right_ = 0;

        // Measured left and right joint states (angular position (rad) and angular velocity (rad/s))
        JointState joint_state_left_;
        JointState joint_state_right_;

        unsigned long encoder_resolution_;

        ros::Subscriber<std_msgs::Empty, BaseController> sub_reset_encoders_;
        ...
        fellowme_msgs::EncodersStamped encoder_msg_;
        ros::Publisher pub_encoders_;

        sensor_msgs::JointState msg_measured_joint_states_;
        ros::Publisher pub_measured_joint_states_;

        L298NController* p_motor_controller_left_;
        L298NController* p_motor_controller_right_;

        ros::Subscriber<fellowme_msgs::WheelsCmdStamped, BaseController> sub_wheel_cmd_velocities_;
        float wheel_cmd_velocity_left_ = 0.0;
        float wheel_cmd_velocity_right_ = 0.0;
        float left_angular_velocity_filter_ = 0.0;
        float left_angular_velocity_prev_ = 0.0;
        float right_angular_velocity_filter_ = 0.0;
        float right_angular_velocity_prev_ = 0.0;

        int motor_cmd_left_ = 0;
        int motor_cmd_right_ = 0;

        ros::Subscriber<fellowme_msgs::PIDStamped, BaseController> sub_pid_left_;
        ros::Subscriber<fellowme_msgs::PIDStamped, BaseController> sub_pid_right_;
        PID motor_pid_left_;
...
using BC = fellowme::BaseController;


fellowme::BaseController
    ::BaseController(ros::NodeHandle &nh, L298NController* motor_controller_left, L298NController* motor_controller_right)
    : nh_(nh)
    ...
    , sub_reset_encoders_("reset", &BC::resetEncodersCallback, this)
    , pub_encoders_("fellowme_base/encoders/ticks", &encoder_msg_)
    , pub_measured_joint_states_("fellowme_base/measured_joint_states", &msg_measured_joint_states_)
    , sub_wheel_cmd_velocities_("wheel_cmd_velocities", &BC::commandCallback, this)
    , last_update_time_(nh.now())
    , update_rate_(UPDATE_RATE_IMU, UPDATE_RATE_CONTROL, UPDATE_RATE_DEBUG)
    , sub_pid_left_("pid_left", &BC::pidLeftCallback, this)
    , sub_pid_right_("pid_right", &BC::pidRightCallback, this)
...
void fellowme::BaseController::setup()
{   
...
void fellowme::BaseController::init()
{
...
void fellowme::BaseController::commandCallback(const fellowme_msgs::WheelsCmdStamped& cmd_msg)
{
    ...
    // Used for the eStop. In case no fellowme_msgs::WheelsCmdStamped messages are received on the wheel_cmd_velocities 
...
void fellowme::BaseController::resetEncodersCallback(const std_msgs::Empty& reset_msg)
{
...
void fellowme::BaseController::pidLeftCallback(const fellowme_msgs::PIDStamped& pid_msg)
{
    // Callback function to update the pid values of the left motor
    // This callback function receives fellowme_msgs::PID message object
    // where fellowme_msgs::PID kp, ki, kd for one pid controller is stored
    motor_pid_left_.updateConstants(pid_msg.pid.kp, pid_msg.pid.ki, pid_msg.pid.kd);
    motor_pid_left_.updateConstants(pid_msg.pid.kp, pid_msg.pid.ki, pid_msg.pid.kd);
}

void fellowme::BaseController::pidRightCallback(const fellowme_msgs::PIDStamped& pid_msg)
{
    // Callback function to update the pid values of the left motor
    // This callback function receives fellowme_msgs::PID message object
    // where fellowme_msgs::PID kp, ki, kd for one pid controller is stored
...
void fellowme::BaseController::read()
{
    joint_state_left_ = encoder_left_.getJointState();
    joint_state_right_ = encoder_right_.getJointState();
    ...
    left_angular_velocity_filter_ = 0.88 * left_angular_velocity_filter_ + 0.06 * joint_state_left_.angular_velocity_ + 0.06 * left_angular_velocity_prev_;
    left_angular_velocity_prev_ = joint_state_left_.angular_velocity_;
    msg_measured_joint_states_.velocity[0] = joint_state_left_.angular_velocity_;

    right_angular_velocity_filter_ = 0.88 * right_angular_velocity_filter_ + 0.06 * joint_state_right_.angular_velocity_ + 0.06 * right_angular_velocity_prev_;
    right_angular_velocity_prev_ = joint_state_right_.angular_velocity_;
    ...
    ticks_left_ = encoder_left_.getTicks();
    ticks_right_ = encoder_right_.getTicks();
...
void fellowme::BaseController::write()
{
    // https://www.arduino.cc/reference/en/language/functions/math/map/
    // map(value, fromLow, fromHigh, toLow, toHigh)
    // Map angular wheel joint velocity to motor cmd
    // The /fellowme/mobile_base_controller/linear/x/max_velocity from the ROS parameter server 
    ...
    ros::Duration dt = nh_.now() - last_update_time_.control;
    float dts = dt.toSec();
    motor_cmd_left_ = motor_pid_left_.compute(wheel_cmd_velocity_left_, joint_state_left_.angular_velocity_, dts);
    motor_cmd_right_ = motor_pid_right_.compute(wheel_cmd_velocity_right_, joint_state_right_.angular_velocity_, dts);

    p_motor_controller_left_->setSpeed(motor_cmd_left_);
    p_motor_controller_right_->setSpeed(motor_cmd_right_);
}

void fellowme::BaseController::eStop()
{
    wheel_cmd_velocity_left_ = 0;
    wheel_cmd_velocity_right_ = 0;
}

void fellowme::BaseController::printDebug()
{
    /*
    String log_msg =
            String("\nRead:\n") +
...
#endif // FELLOWME_BASE_CONTROLLER_H